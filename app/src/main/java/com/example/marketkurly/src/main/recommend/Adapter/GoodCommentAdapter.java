...
public class GoodCommentAdapter extends RecyclerView.Adapter<GoodCommentAdapter.CustomViewHolders> {


    Context mContext;
    private ArrayList<GoodCommentData> arrayList;
    ...
    public GoodCommentAdapter(ArrayList<GoodCommentData> arrayList) {
        this.arrayList = arrayList;
    }

    @NonNull
    @Override
    public GoodCommentAdapter.CustomViewHolders onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        View view = LayoutInflater.from(parent.getContext()).inflate(R.layout.item_product_best_comment, parent, false);
    ...
    public void onBindViewHolder(@NonNull final CustomViewHolders holder, final int position) {
        ...
        //flipperView
        View flipperview[] = new View[arrayList.get(position).getReviewList().size()];
        for (int i=0; i<arrayList.get(position).getReviewList().size(); i++) {
            flipperview[i] = (View) View.inflate(holder.itemView.getContext(), R.layout.item_viewflipper,null);
            TextView tv_review = (TextView) flipperview[i].findViewById(R.id.view_flipper_review);
            TextView tv_name = (TextView) flipperview[i].findViewById(R.id.view_flipper_name);
            tv_review.setText((CharSequence) arrayList.get(position).getReviewList().get(i));
            tv_name.setText((CharSequence) arrayList.get(position).getNameList().get(i));
            holder.viewFlipper.addView(flipperview[i]);
            holder.viewFlipper.setFlipInterval(2000);
            holder.viewFlipper.startFlipping(); //언제 어디서 끝내야 하지...?ㅎㅎ
        }
    ...
    public class CustomViewHolders extends RecyclerView.ViewHolder {
        ...
        //protected View flipperview[] = new View[4];
        ...
        public CustomViewHolders(@NonNull View itemView) {
            ...
            this.viewFlipper = (ViewFlipper) itemView.findViewById(R.id.view_flipper);
